...
def main(config):
    ...
    mask_rotation = float(config['thresholding']['mask_rotation'])
    ...
    logger.info("mask_height: {0}  mask_width: {1} mask_rotation: {2}".format(mask_height, mask_width, mask_rotation))
    ...
    for image_idx in range(len(image_files)):
        ...
        if 'thresholded' in short_file_name or not short_file_name.endswith('_ok') or file_ext != '.fits':
