...
class RadialProfile(object):
    ...
    def __init__(self, image_shape, bin_size):
        ...
        self.cached_r = r
        self.cached_bins = bins
        self.cached_hist = hist.astype(np.float)


    def azimuthal_average_new(self, image):

        #radial_prof = np.histogram(r, bins, weights=(image))[0] / hist
        radial_prof = np.histogram(self.cached_r, self.cached_bins, weights=(image))[0] / self.cached_hist

        return radial_prof

    def azimuthal_average(self, image, center=None, stddev=False, returnradii=False, return_nr=False,
        ...
        if stddev:
            ...
            hist = np.histogram(r, bins, weights=(mask*weights))[0]
            for i in range(hist.shape[0]):
                if hist[i] == 0:
                    hist[i] = 1.
            #radial_prof = np.histogram(r, bins, weights=(image*weights*mask))[0] / np.histogram(r, bins, weights=(mask*weights))[0]
            radial_prof = np.histogram(r, bins, weights=(image*weights*mask))[0] / hist
